<template>
  <div class="bg-[#f4ece6] min-h-screen">
    <TheHeader />
    <PageHero title="Rooms & Suites" :image="heroImg" subtitle="Accommodation" />
    ...
    <div class="max-w-7xl mx-auto px-6 py-16">
      <div v-if="rooms.length">
        <div v-for="(room, i) in rooms" :key="room.id"
          ...
          <!-- Image -->
          <div class="relative md:w-[58%] overflow-hidden bg-[#e8d5c0] group">
            <div class="aspect-[4/3] md:aspect-auto md:h-full min-h-[340px] overflow-hidden">
              <img :src="getCurrentImage(room)" :alt="room.name"
            ...
            <template v-if="getImages(room).length > 1">
              <button @click.prevent="prevImage(room)"
                ...
                <Icon name="mdi:chevron-left" class="text-[#1a1410] text-xl" />
              </button>
              <button @click.prevent="nextImage(room)"
                ...
                <Icon name="mdi:chevron-right" class="text-[#1a1410] text-xl" />
              </button>
              <div class="absolute bottom-4 left-0 right-0 flex justify-center gap-1.5 z-10">
                <button v-for="(_, idx) in getImages(room)" :key="idx"
          ...
          <div class="md:w-[42%] flex flex-col justify-center px-10 py-14"
            ...
            <p v-if="room.category" class="text-[10px] uppercase tracking-[0.3em] text-[#b7825c] mb-4">{{ room.category }}</p>
            <h2 class="font-['Cormorant_Garamond'] text-4xl sm:text-5xl text-[#1a1410] font-light mb-6 leading-tight">{{ room.name }}</h2>
            <div class="flex items-center gap-8 mb-6 text-[#1a1410]/50 text-sm">
              <span v-if="room.size" class="flex items-center gap-2">
                <Icon name="mdi:crop-square" class="text-base flex-shrink-0" />{{ room.size }} m²
              </span>
              <span v-if="room.guests" class="flex items-center gap-2">
                <Icon name="mdi:account-multiple-outline" class="text-base flex-shrink-0" />{{ room.guests }}
              </span>
              <span v-if="room.bedType" class="flex items-center gap-2">
                <Icon name="mdi:bed-outline" class="text-base flex-shrink-0" />{{ room.bedType }}
              </span>
            </div>
            <div class="w-8 h-px bg-[#b7825c] mb-6"></div>
            <p class="text-[#1a1410]/55 text-sm leading-relaxed mb-10">{{ room.description || room.shortDescription || '' }}</p>
            <div class="flex items-center gap-8">
              ...
              <NuxtLink :to="localePath(`/rooms/${room.slug}`)"
                class="text-[#1a1410] text-[10px] uppercase tracking-[0.2em] border-b border-[#1a1410]/30 pb-0.5 hover:text-[#b7825c] hover:border-[#b7825c] transition-colors">
                More Information
              </NuxtLink>
            </div>
          </div>
...
const getImages      = (room) => room.images?.length ? room.images : room.thumbnail || room.image ? [room.thumbnail || room.image] : []
...
const nextImage      = (room) => { const imgs = getImages(room); imageIndexes[room.id] = (getActiveIndex(room) + 1) % imgs.length }
const prevImage      = (room) => { const imgs = getImages(room); imageIndexes[room.id] = (getActiveIndex(room) - 1 + imgs.length) % imgs.length }

onMounted(async () => {
  const { fetchGallery, fetchRooms } = useHotel()
  const [roomsData, gallery] = await Promise.all([fetchRooms(locale.value), fetchGallery()])
